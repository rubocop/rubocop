...
| Progress (default) | `progress` | Interactive terminal use
| Clang | `clang` | Detailed offenses with source context
| Simple | `simple` | Compact human-readable output
| Quiet | `quiet` | Only show output when there are offenses
| Fuubar | `fuubar` | Progress bar with real-time offense reporting
| Pacman | `pacman` | Fun progress indicator
| Emacs | `emacs` | Emacs-compatible output (machine-parsable)
| JSON | `json` | Machine consumption / custom tooling (machine-parsable)
...
| Offense Count | `offenses` | Overview of which cops trigger the most
| Worst Offenders | `worst` | Find files needing the most attention
|===
...
You can also load xref:development.adoc#custom-formatters[custom formatters].
...
Useful for CI environments. It will create an HTML report like https://github.com/rubocop/rubocop/blob/master/spec/fixtures/html_formatter/expected.html[this].
