...
* A vast number of ready-made xref:plugins.adoc[extensions] (e.g. `rubocop-rails`, `rubocop-rspec`, `rubocop-performance` and `rubocop-minitest`)
* Built-in xref:usage/lsp.adoc[Language Server Protocol (LSP)] and xref:usage/mcp.adoc[Model Context Protocol (MCP)] support
...
* Start with the xref:usage/getting_started.adoc["Getting Started"] guide to get up and running quickly.
* See xref:usage/cli_reference.adoc["CLI Reference"] for the full command-line reference.
...
* Explore the xref:plugins.adoc[available plugins].
