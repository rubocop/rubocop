...
Full reference for the `rubocop` command-line interface. For a tutorial introduction, see xref:usage/getting_started.adoc[Getting Started].
...
| `-r/--require`
| Require Ruby file (see xref:plugins.adoc#loading-legacy-extensions-via-require[Loading Legacy Extensions]).
...
| Also report `.rubocop_todo.yml` `Exclude` entries that are no longer needed - files the cop would not flag anymore, or files that no longer exist - and fail the run when any are found. Unlike `--regenerate-todo` it never absorbs new offenses, which makes it safe as a CI ratchet; see xref:usage/auto_gen_config.adoc#keeping-the-todo-from-rotting[Keeping the TODO from rotting].
...
| `--explain COP1,COP2,...`
| Explain what the given cops do: their description, properties, configuration, the examples from their own documentation, and links to the style guide and docs. Unlike `--show-cops` and `--show-docs-url`, an unrecognized name is reported rather than skipped.
