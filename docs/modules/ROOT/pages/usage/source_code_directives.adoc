...
A reason is optional, but a project can require one. `Lint/MissingCopDisableReason`
(disabled by default) reports any `disable` that does not explain itself, which
turns a suppression into a record of a decision rather than a bare fact:

[source,yaml]
----
Lint/MissingCopDisableReason:
  Enabled: true
----

`rubocop:todo` directives are exempt, since `--auto-gen-config` and
`--disable-uncorrectable` generate them and "this is debt" is explanation
enough. Neither `rubocop:disable all` nor `rubocop:disable Lint` silences the cop - a
blanket disable with no explanation is the case it most wants to report - so it
has to be named to be turned off.
