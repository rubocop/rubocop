...
`rubocop:disable`. The corresponding next-statement and whole-file aliases
are `rubocop:todo-next` and `rubocop:todo-file`.
...
== Disabling Cops for the Whole File

An explicit `disable-file` directive suppresses the listed cops for the whole
physical file, from the first line through the end of the file. It has no
matching `enable`: later `enable`, `enable-next`, or `pop` directives do not
narrow its scope.
...
# rubocop:disable-file Metrics/MethodLength, Style/StringLiterals -- generated file
----

Place a file directive on its own line in the file header, before any Ruby
code. A shebang, magic comments, other comments, and blank lines may precede
it. A misplaced file directive still suppresses the whole physical file, but
`Lint/CopDirectiveSyntax` reports its placement so that the directive's scope
is visible where readers expect it.

Like other disabling directives, `disable-file` accepts individual cops,
departments, `all`, comma-separated lists, and a trailing `--` reason.
`todo-file` is the equivalent of `rubocop:todo`. `--auto-gen-config` never
generates file directives, so both forms are only ever written by hand.
...
* *`disable-file`* - the whole physical file. Put it in the file header; it
  has no closing directive and cannot be narrowed later.

`todo`, `todo-next` and `todo-file` are aliases of `disable`, `disable-next`
and `disable-file` that mark a suppression you intend to revisit rather than
a deliberate exception.

The `Style/DirectiveScope` cop can enforce the tightest-form preference: it
flags `disable`/`enable` and `enable`/`disable` pairs as well as `push`/`pop`
...
| A directive RuboCop cannot act on: a missing or misspelled mode, cop names
  ...
  exist. It also reports next-statement directives at the end of a code line
  and file directives outside the file header. Malformed directives suppress
  nothing silently; misplaced file directives retain whole-file scope but are
  reported because their placement obscures it.
| Enabled
...
| A `disable`, `disable-next`, or file directive that is no longer suppressing
  anything - the offense it was hiding is gone. Autocorrection removes the
  directive along with its `--` reason.
| Enabled
...
| A `disable` that is never closed, so it runs to the end of the file.
  ...
  reported. An explicit `disable-file` scope never requires an enable and is
  not reported.
| Enabled
