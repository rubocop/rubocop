...
If you have a code base with an overwhelming amount of offenses, you can use `rubocop --auto-gen-config` to generate a `.rubocop_todo.yml` that disables all current offenses. See xref:usage/auto_gen_config.adoc[Auto-generating Configuration] for details.
