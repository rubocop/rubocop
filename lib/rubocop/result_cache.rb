...
module RuboCop
  ...
  class ResultCache
    ...
    def self.cleanup(config_store, verbose, cache_root_override = nil)
      ...
      cache_working_dir = cache_root(config_store, cache_root_override)
      return unless File.exist?(cache_working_dir)

      files, dirs = Find.find(cache_working_dir).partition { |path| File.file?(path) }
      return unless requires_file_removal?(files.length, config_store)

      remove_oldest_files(files, dirs, cache_working_dir, verbose)
    end
    ...
    class << self
      ...
      def remove_oldest_files(files, dirs, cache_working_dir, verbose)
        # Add 1 to half the number of files, so that we remove the file if
        # there's only 1 left.
        remove_count = (files.length / 2) + 1
        puts "Removing the #{remove_count} oldest files from #{cache_working_dir}" if verbose
    ...
    def initialize(file, team, options, config_store, cache_root_override = nil)
      ...
      cache_working_dir = ResultCache.cache_root(config_store, cache_root_override)
      @allow_symlinks_in_cache_location =
        ResultCache.allow_symlinks_in_cache_location?(config_store)
      @path = File.join(cache_working_dir,
                        rubocop_checksum,
