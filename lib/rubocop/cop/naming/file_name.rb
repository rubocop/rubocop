  ...
  module Cop
    module Naming
      ...
      class FileName < Base
        ...
        MSG_SNAKE_CASE = 'The name of this source file (`%{basename}`) should use snake_case.'
        MSG_NO_DEFINITION = '`%{basename}` should define a class or module called `%{namespace}`.'
        MSG_REGEX = '`%{basename}` should match `%{regex}`.'
