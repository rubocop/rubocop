  ...
  module Cop
    module Style
      ...
      class MissingElse < Base
        ...
        MSG = '`%{type}` condition requires an `else`-clause.'
        MSG_NIL = '`%{type}` condition requires an `else`-clause with `nil` in it.'
        MSG_EMPTY = '`%{type}` condition requires an empty `else`-clause.'
