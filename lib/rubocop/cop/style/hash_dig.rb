...
module RuboCop
  module Cop
    module Style
      # Use `Hash#dig` instead of chaining potentially null lookups.
      #
      ...
      # multiple arguments.
      #
      # With `fetch`, the 2nd parameter is allowed to be `{}` or `Hash.new` as long as the
      # final call in the chain is a nil value. If a non-nil value is given, the chain
      # will not be registered as an offense, as the default value cannot be given with `dig`.
      #
      ...
      #   hash.dig('foo', 'bar')
      #
      #   # good - not handled by the cop since the final `fetch` value is non-nil
