  ...
  module Cop
    module Rails
      ...
      class HasManyOrHasOneDependent < Cop
        ...
        def_node_matcher :present_has_many_or_has_one_with_options?, <<-PATTERN
          (send nil? {:has_many :has_one} _ (hash $...))
        PATTERN

        def_node_matcher :present_has_dependent?, <<-PATTERN
          (pair (sym :dependent) !nil)
        PATTERN

        def_node_matcher :present_has_through?, <<-PATTERN
        ...
        def on_send(node)
          if !present_has_many_or_has_one_without_options?(node)
            options = present_has_many_or_has_one_with_options?(node)
            return if valid_options?(options)
        ...
        def valid_options?(options)
          ...
          return true if options.any? do |o|
            present_has_dependent?(o) || present_has_through?(o)
          end
