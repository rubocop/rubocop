...
RSpec.describe RuboCop::ResultCache, :isolated_environment do
  ...
  let(:cache_working_dir) { "#{Dir.pwd}/.cache/rubocop_cache" }
  let(:offenses) do
  ...
  describe '.cleanup' do
    ...
    it 'removes the oldest files in the standard cache_root if needed' do
      cache.save(offenses)
      underscore_dir = Dir["#{cache_working_dir}/*/*"].first
      ...
      expect($stdout.string).to eq("Removing the 2 oldest files from #{cache_working_dir}\n")
    end

    it 'removes the oldest files in a custom cache_root if needed' do
      custom_cache_root = "#{Dir.pwd}/.custom_cache"
      custom_cache_working_dir = "#{Dir.pwd}/.custom_cache/rubocop_cache"
      ...
      underscore_dir = Dir["#{custom_cache_working_dir}/*/*"].first
      expect(Dir["#{underscore_dir}/*"].size).to eq(2)

      custom_cache.class.cleanup(config_store, :verbose, custom_cache_root)
      expect(File).not_to exist(underscore_dir)
      expect($stdout.string).to eq("Removing the 2 oldest files from #{custom_cache_working_dir}\n")
    end
  end
